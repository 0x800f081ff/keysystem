...
export default async function handler(req, res) {
  ...
  const { token, action, user_id, license_key, allowed_uses, days_valid, email_locked } = req.body;
  ...
  try {
    ...
    if (action === 'generate') {
      ...
      await pool.query(
        ...
        [key, allowed_uses || 1, email_locked || 0, expiry]
      );
      msg = `License generated: ${key}`;
    }
    // Ban/unban user and linked licenses
    ...
    const licenses = await Promise.all(
      licensesRaw.map(async l => {
        ...
        // rename fields to remove hwid references
        l.email = l.hwid || null;
        delete l.hwid; // remove old field completely
        return l;
      })
    );
